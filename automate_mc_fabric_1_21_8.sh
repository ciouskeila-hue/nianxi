...
: >"$LOG_FILE"
rm -rf "$WORK_DIR/saves" "$WORK_DIR/screenshots"
...
van=json.load(open(main_dir/'versions/1.21.8/1.21.8.json'))
fab=json.load(open(main_dir/'versions/fabric-1.21.8-0.18.4/fabric-1.21.8-0.18.4.json'))
assets=json.load(open(main_dir/'assets/indexes/26.json'))
jobs=[]
...
Xvfb :99 -screen 0 1280x720x24 +extension GLX +render -noreset >/tmp/xvfb-mc.log 2>&1 &
XVFB_PID=$!
